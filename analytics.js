...
(function (g) {
  ...
  try {
    ...
    if (justOff) localStorage.setItem('bh.an.off', '1');
    if (localStorage.getItem('bh.an.off') === '1') {
  ...
  var isNew = 0, vid = '', sid = '';
  try {
    vid = localStorage.getItem('bh.an.v') || '';
    if (!vid) { vid = rnd(); isNew = 1; localStorage.setItem('bh.an.v', vid); }
  ...
  sid = ss('bh.an.s') || '';
  var T0 = Number(ss('bh.an.t0')) || 0;
  var seq = Number(ss('bh.an.q')) || 0;
  var freshSession = !sid;
  if (!sid) {
    sid = rnd() + rnd().slice(0, 4); T0 = Date.now(); seq = 0;
    ss('bh.an.s', sid); ss('bh.an.t0', T0);
  }
  if (!T0) T0 = Date.now();          /* 저장이 반쪽만 남은 이상한 상태 대비 */
  var views = Number(ss('bh.an.n')) || 0;
  ...
  g.BWA = {
    ...
    view: function (name, key, title) { views++; ss('bh.an.n', views); lastView = name; push('view', name, key, title); },
