...
# 주간업무계획은 다음 주 것이 미리 올라와 D.weekStart 가 오늘을 안 품을 때가 있다.
# 그대로 두면 '1주' 보기가 다음 주가 되어 뉴스·행사가 0건으로 보인다(실제로 그랬다).
must "$P" "const HOME_WEEK"      "군민용: '이번 주'가 오늘이 든 주 기준"
must "$P" "let VIEW_WEEK=HOME_WEEK" "군민용: 첫 화면이 오늘이 든 주로 열림"
if grep -q "VIEW_WEEK=D.weekStart; S.scope='week'" "$P"; then
  no "군민용: '1주' 되돌리기가 자료 기준 주(다음 주일 수 있음)로 되돌아갔습니다"
else ok "군민용: '1주'가 오늘이 든 주로 되돌아감"; fi
...
must "$DEPLOY/ui.js" "전에 올라왔지만 아직 신청할 수 있어요" "엔진: 이어지는 소식 구분"
